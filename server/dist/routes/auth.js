...
authRouter.get("/github/callback", async (req, res, next) => {
    try {
        ...
        const user = await User.findOneAndUpdate({ githubId: githubUser.githubId }, { ...githubUser, accessToken }, { upsert: true, new: true });
...
authRouter.put("/starred/:owner/:repo", requireAuth, async (req, res, next) => {
    try {
        ...
        await User.updateOne({ _id: req.user._id }, { $addToSet: { starredRepos: fullName } });
...
authRouter.delete("/starred/:owner/:repo", requireAuth, async (req, res, next) => {
    try {
        const fullName = `${req.params.owner}/${req.params.repo}`;
        await User.updateOne({ _id: req.user._id }, { $pull: { starredRepos: fullName } });
